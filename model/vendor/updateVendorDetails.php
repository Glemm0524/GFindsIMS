	...
	if(isset($_POST['vendorDetailsVendorID'])) {
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				// Validate mobile number (as a string of digits)
				if(preg_match('/^\d{10,15}$/', $vendorDetailsVendorMobile)) {
				...
				// Validate second phone number only if it's provided by user (optional)
				if(!empty($vendorDetailsVendorPhone2)) {
					// Validate phone number 2 (allowing 10 to 15 digits, including leading zero)
					if(!preg_match('/^\d{10,15}$/', $vendorDetailsVendorPhone2)) {
